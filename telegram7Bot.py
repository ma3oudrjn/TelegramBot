...
def start(message):
    menu = create_menu()
    bot.send_message(message.chat.id, "Welcome! Please choose an option from the menu.", reply_markup=menu)
...
def save_user_info(message, course):
    ...
    if len(name_contact) >= 2:
        ...
        bot.send_message(message.chat.id, "Invalid input. Please provide your name and contact information in separate lines.")
